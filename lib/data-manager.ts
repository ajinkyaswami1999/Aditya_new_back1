...
export const projectsApi = {
  ...
  async create(project: Omit<Project, 'id' | 'created_at' | 'updated_at'>): Promise<Project> {
    ...
    // Fallback: Add to demo data with generated ID for current session
    const newProject: Project = {
      ...projectData,
      ...
      if (await isSupabaseConnectedWithRetry() && supabaseServerClient) {
        ...
        if (!error && data?.value) {
          ...
          if (error?.code === 'PGRST116' || error?.message?.includes('No rows found')) {
            console.log(`[DataManager] ${key} not found in database, this might be expected for new settings`);
            // Return fallback data immediately - don't try to create here
            const fallbackValue = (fallbackData.site_settings as any)[key];
            console.log(`[DataManager] Using fallback data for ${key}:`, fallbackValue);
            return fallbackValue;
          }
          throw new Error(`Failed to load ${key} from database: ${error?.message || 'Unknown error'}`);
        }
