...
export const siteSettingsApi = {
  ...
  async set(key: keyof SiteSettings, value: any): Promise<void> {
    console.log(`[DataManager] Setting site setting: ${key}`, value);
    try {
      const isConnected = await isSupabaseConnectedWithRetry();
      ...
      if (isConnected && supabaseAdmin) {
        ...
        console.error(`[DataManager] Supabase not available for updating ${key}. Connected: ${isConnected}, Admin client: ${!!supabaseAdmin}`);
